...
public class ConfigParser {
	...
	public static KeyType parseKeyType(String string) throws Exception {
		...
		if(string.matches("[0-9a-zA-Z]*")) {
			...
			throw new Exception(" Key contains invalid characters: " + string);
	...
	public static List<Effect> parseEffects(FileConfiguration config, String path) throws Exception {
		...
		if(!config.isConfigurationSection(path) || config.getConfigurationSection(path).getKeys(false).isEmpty())
			return effects;
		for(String effect_ : config.getConfigurationSection(path).getKeys(false)) {
			...
			if(Util.isAllowedWithTarget(efftyp, efftar))
				...
				throw new Exception("The effect type " + efftyp.toString().toLowerCase() + " is not allowed to have the target " + efftar);
		}
		return effects;
	}
	...
	private static EffectTargetImpl buildEffectTarget(FileConfiguration f, String path) {
		...
		case TARGETLOCATION:
			...
			break;
		case FLIGHTPATH:
			efftar.addProperty("LENGTH", new NumberProperty(((Integer) (searchKeyInMapList(args, "length").get("length")))));
			break;
